...
void file::open() {/*交互式*/
	...
	if (ch == '0') {
		...
		string fname;
		getline(cin, fname);
		cout << "是否追加到文件尾？（YES or NO）\n";
		string appif;
		cin >> appif;
		while (appif != "YES" && appif != "NO") {
			cin >> appif;
		}
		ofstream fout;
		if (appif == "YES") {
...
/*程序式*/
std::vector<std::string> file::open(std::string fname, MODE mod, bool ifapp, std::vector<std::string>out, int idofout) {
	using namespace std;
	vector<string>err;
	err.push_back("ERROR");
	switch (mod) {
	...
	case ALL: {
		...
		ifstream fin;
		if (ifapp) {
			fin.open(fname, ios::in | ios::out | ios::app );
		}
		else {
...
bool file::Delete(std::string file_name) {
